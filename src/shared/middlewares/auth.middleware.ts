...
export const isAuthenticated = async (
  ...
  if (authHeader) {
    ...
    try {
      ...
      return next(
        new CustomError("invalid token", "AUTH", HttpErrorStatus.Unauthorized)
      );
    }
  }
  return next(
...
export const checkRole = (...roles: Role[]) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!roles.includes(req.user.role)) {
      return next(
        new CustomError("Forbidden", "AUTH", HttpErrorStatus.Forbidden)
      );
    }
    next();
  };
...
export const isAuthorized = async (
  ...
  if (!Number.isFinite(id)) {
    return next(
      new CustomError("Invalid course id", "COURSE", HttpErrorStatus.BadRequest)
    );
  }
  ...
  if (!course) {
    return next(
      new CustomError("Course not found", "COURSE", HttpErrorStatus.NotFound)
    );
  }

  if (user.id !== course.creatorId) {
    return next(
      new CustomError("Unauthorized", "COURSE", HttpErrorStatus.Unauthorized)
    );
  }
  return next();
};
